...
async function getProducts(req, res) {
  try {
    ...
    if (sale === 'true') ref = ref.where('badge', '==', 'sale');
...
async function getProduct(req, res) {
  try {
    ...
    if (snap.empty) {
      ...
      if (!byId.exists) return res.status(404).json({ success: false, message: 'Product not found.' });
...
async function getCategories(req, res) {
  try {
    const snap = await db().collection('categories').where('active', '==', true).orderBy('sortOrder', 'asc').get();
    const cats = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    if (cats.length === 0) {
      // Return defaults if Firestore is empty
...
// POST /api/products/:id/review — buyer only
async function addReview(req, res) {
  try {
    ...
    const review = {
      id: reviewId, productId: id, buyerId,
