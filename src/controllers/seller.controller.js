  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        lowStockProducts,
        storeName: store.name || req.user.name || 'My Store',
...
async function createProduct(req, res) {
  try {
    const sellerId = req.user.id || req.user.uid;
    const {
      name, description, price, oldPrice, stock, sku,
      categoryId, images = [], badge, tags = [], featured = false,
    } = req.body;
    ...
    const slug = slugify(name) + '-' + Math.floor(parseFloat(price));
    ...
    const productData = {
      ...
      sellerId,
      sellerName: storeData.name || userData.name || 'Unknown Seller',
      status: 'pending',
      rating: 0,
      reviewCount: 0,
    ...
    res.status(201).json({
      success: true,
      message: 'Product submitted for review. It will go live once approved by admin.',
      data: productData,
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
...
async function updateProduct(req, res) {
  try {
    ...
    if (name !== undefined) {
      updates.name = name;
      updates.slug = slugify(name) + '-' + Math.floor(parseFloat(price || snap.data().price));
    }
    ...
    if (name !== undefined || price !== undefined) updates.status = 'pending';
...
async function deleteProduct(req, res) {
  try {
    ...
    } catch (e) { /* ignore if store doesn't exist */ }
...
async function updateOrderStatus(req, res) {
  try {
    ...
    if (!order.sellerIds?.includes(sellerId) && req.user.role !== 'admin') {
...
async function updateStore(req, res) {
  try {
    ...
    const storeRef = db().collection('stores').doc(sellerId);
    const snap = await storeRef.get();
    if (!snap.exists) {
      await storeRef.set({ id: sellerId, ownerId: sellerId, ...updates, createdAt: new Date().toISOString() });
    } else {
...
module.exports = {
  getDashboard, getSellerProducts, createProduct, updateProduct,
  deleteProduct, getSellerOrders, updateOrderStatus,
  verifyBankAccount, getStore, updateStore,
};
