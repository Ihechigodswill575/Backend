...
async function seed() {
  ...
  const coupons = [
    ...
    { code: 'WELCOME', type: 'percentage', value: 15, minOrder: 3000, maxUses: 200, description: '15% off welcome discount' },
  ];
  ...
  await db().collection('config').doc('platform').set({
    ...
    supportEmail: 'support@tavikmart.com',
  ...
  console.log(`  URL:      /pages/admin.html`);
  console.log(`  Email:    ${adminEmail}`);
  console.log(`  Password: ${adminPassword}`);
  console.log('='.repeat(50));
  process.exit(0);
