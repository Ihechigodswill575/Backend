...
async function getDashboard(req, res) {
  try {
    ...
    const revenue = paidOrders.reduce((s, o) => s + (o.commission || Math.round((o.subtotal || o.total || 0) * 0.05)), 0);
...
async function updateProductStatus(req, res) {
  try {
    ...
    const validStatuses = ['approved', 'rejected', 'pending', 'out_of_stock'];
...
async function createCategory(req, res) {
  try {
    ...
    const data = {
      id, name, slug: id, icon: icon || '📦',
      description: description || '',
      active: true, productCount: 0, sortOrder: 99,
...
async function createCoupon(req, res) {
  try {
    const { code, type, value, minOrder = 0, maxUses = 1000, expiresAt, description } = req.body;
    if (!code || !type || !value) {
...
module.exports = {
  getDashboard, getUsers, updateUser, getProducts, updateProductStatus,
  getOrders, updateOrderStatus, getCategories, createCategory,
  updateCategory, deleteCategory, createCoupon, getCoupons,
};
