...
async function updateCartItem(req, res) {
  try {
    ...
    if (!cartItemId || quantity == null) {
      return res.status(400).json({ success: false, message: 'cartItemId and quantity required.' });
    }
...
async function removeFromCart(req, res) {
  try {
    ...
    if (!cartItemId) return res.status(400).json({ success: false, message: 'cartItemId required.' });
