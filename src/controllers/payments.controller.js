...
async function paystackInitialize(req, res) {
  try {
    ...
    const response = await axios.post(`${PAYSTACK_BASE}/transaction/initialize`, {
      ...
      callback_url: callbackUrl || `${process.env.FRONTEND_URL}/pages/order-confirm.html?id=${orderId}`,
      metadata: {
        orderId,
        buyerId: req.user.id || req.user.uid,
        cancel_action: `${process.env.FRONTEND_URL}/pages/cart.html`,
      },
    ...
    res.json({
      success: true,
      data: { authorizationUrl: authorization_url, reference, accessCode: access_code },
    });
  } catch (err) {
...
async function paystackVerify(req, res) {
  try {
    ...
    if (!orderId) {
      return res.status(400).json({ success: false, message: 'Order reference missing from payment metadata.' });
    }
    ...
    res.json({
      ...
      data: { orderId, amount: amount / 100, reference },
    });
  } catch (err) {
    const msg = err.response?.data?.message || err.message;
...
// NOTE: This route receives raw body (Buffer) — must be registered BEFORE express.json()
...
async function flutterwaveInitialize(req, res) {
  try {
    ...
    const response = await axios.post(`${FLUTTERWAVE_BASE}/payments`, {
      ...
      amount,
      currency: 'NGN',
      redirect_url: callbackUrl || `${process.env.FRONTEND_URL}/pages/order-confirm.html?id=${orderId}`,
      customer: { email, name, phonenumber: phone },
      customizations: {
        title: 'TAVIKMART',
        logo: `${process.env.FRONTEND_URL}/assets/logo.svg`,
      },
      meta: { orderId },
    ...
    await ordersRef.update({
      transactionRef: `TVM-FLW-${orderId}`,
      updatedAt: new Date().toISOString(),
    });
    ...
    const msg = err.response?.data?.message || err.message;
    res.status(500).json({ success: false, message: `Flutterwave init failed: ${msg}` });
  }
}
...
async function flutterwaveWebhook(req, res) {
  try {
    ...
    // req.body is a Buffer for this route
    ...
    if (payload.event === 'charge.completed' && payload.data?.status === 'successful') {
      ...
      if (orderId) {
        await db().collection('orders').doc(orderId).update({
          paymentStatus: 'paid',
          orderStatus: 'confirmed',
          paidAmount: payload.data.amount,
          paidAt: new Date().toISOString(),
          updatedAt: new Date().toISOString(),
        });
        console.log(`✅ Order ${orderId} paid via Flutterwave webhook`);
      }
    }
...
async function verifyAccount(req, res) {
  try {
    ...
    res.status(400).json({ success: false, message: err.response?.data?.message || 'Account verification failed.' });
  }
}
